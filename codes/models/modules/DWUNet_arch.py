...
class DCR_block(nn.Module):
    def __init__(self, channel_in):
        ...
        self.conv_1 = nn.Conv2d(in_channels=channel_in, out_channels=channel_in, kernel_size=3, stride=1, padding=1)
        self.conv_2 = nn.Conv2d(in_channels=channel_in * 2, out_channels=channel_in, kernel_size=3, stride=1, padding=1)
...
class Down(nn.Module):
    def __init__(self, channel_in, channel_out):
        ...
        self.conv = nn.Conv2d(channel_in * 4, channel_out, kernel_size=3, stride=1, padding=1)
...
class Up(nn.Module):
    def __init__(self, channel_in, channel_out):
        ...
        self.dcr = nn.Sequential(DCR_block(channel_in),
                                 DCR_block(channel_in))
        self.conv1 = nn.Conv2d(channel_in//4, channel_in//2, kernel_size=3, stride=1, padding=1)
...
class DWUNet(nn.Module):
    def __init__(self):
        ...
        self.outc = nn.Conv2d(in_channels=64, out_channels=3, kernel_size=3, stride=1, padding=1)
        self.relu = nn.PReLU()
    ...
    def forward(self, x):
        ...
        out = torch.add(self.relu(out), residual)
        # out = torch.sigmoid(self.upsample(self.convf(out)))

        return out

    class DWUNet_old(nn.Module):
        def __init__(self):
            super(DWUNet_old, self).__init__()
